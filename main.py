...
# ✅ Wrapper for tf.reduce_mean to keep Keras symbolic
class MeanAdvantageLayer(layers.Layer):
    def call(self, advantage):
        mean = tf.reduce_mean(advantage, axis=1, keepdims=True)
        return advantage - mean

def create_dueling_conv1d_q_model(input_shape, num_actions):
    ...
    # Combine streams
    normalized_advantage = MeanAdvantageLayer()(advantage)
    q_values = layers.Add()([value, normalized_advantage])
...
def main():
    ...
    for params in param_grid_search(param_grid):
        ...
        model = create_dueling_conv1d_q_model(input_shape, num_actions)
        model_target = create_dueling_conv1d_q_model(input_shape, num_actions)
